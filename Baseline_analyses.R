...
###
### Preparation of BORDER 10 micron, 12 clusters for further analysis ----
###
...
## Loading and changing raw counts matrix
...
###
### Calculate numbers for the manuscript
###
...
##
## Performing a multivariate COX regression analysis
##
# Convert hrcyto to a factor, if it isn't already
survival_bl_border10_cn12$hrcyto <- as.factor(survival_bl_border10_cn12$hrcyto)

# Relevel hrcyto so that "HR" is the reference level
survival_bl_border10_cn12$hrcyto <- relevel(survival_bl_border10_cn12$hrcyto, ref = "SR")
# CN associated with PFS: CN0, CN11, CN8 (all neg), CN9 (pos)

border10cn12_pfs_mva_allCN <- coxph(Surv(PFS_days, PFS_event) ~ survival_bl_border10_cn12$CN0
                              + survival_bl_border10_cn12$CN11
                              + survival_bl_border10_cn12$CN8
                              + survival_bl_border10_cn12$CN9 
      + survival_bl_border10_cn12$EMD
      + survival_bl_border10_cn12$Ferritin_D0_log
      + survival_bl_border10_cn12$hrcyto
...
# Generating different models for each CN
border10cn12_pfs_mva_allCN 
...
coxph(Surv(PFS_days, PFS_event) ~ #ifelse(survival_bl_border10_cn12$CN0 > median(survival_bl_border10_cn12$CN0), 1, 0)
...
###
### COMPOSITION OF NEIGHBORHOODS IN BORDER10CN12 ----
###
...
### MYELOMA COMPARTMENT
###
p_blborder10cn12_myeloma <- bl_border10cn12_raw |>
  # filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="Myeloma\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  coord_flip()+
  theme_classic()+
  theme(legend.position = "top")

p_blborder10cn12_myeloma_ki67 <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_myeloma_pdl1 <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_myeloma_neg <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
##
## NEW CD4 ANALYSIS INCLUDING Chi2 TEST
##

# Original data processing
cn_cd4_fractions <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  group_by(cn_celltypes) %>%
...
# Create combined data
all_cd4_fractions <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  count(minor_cell_type) %>%
  filter(minor_cell_type %in% c("CD4 Tnaive","Non Th17 CD4 Tmem", "Th17 CD4 Tmem", "CD4 Tex", "Treg")) %>%
  mutate(
...
# Combine original and combined data
cd4_fractions <- bind_rows(cn_cd4_fractions, all_cd4_fractions)

# Get total counts across all cn_celltypes
total_cd4_counts <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  filter(minor_cell_type %in% c("CD4 Tnaive","Non Th17 CD4 Tmem", "Th17 CD4 Tmem", "CD4 Tex", "Treg")) %>%
  count(minor_cell_type) %>%
  rename(total_n = n)
...
# Get counts per cn_celltypes
cn_cd4_counts <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  filter(minor_cell_type %in% c("CD4 Tnaive","Non Th17 CD4 Tmem", "Th17 CD4 Tmem", "CD4 Tex", "Treg")) %>%
  group_by(cn_celltypes) %>%
  count(minor_cell_type) %>%
...
# Perform Fisher's exact test for CNS of interest
cn_list <- c("0","8","9","11")
cd4_fisher_results <- list()

for (cn in cn_list) {
  # Get counts for the current cn_celltypes
  cn_data <- cn_cd4_counts %>%
    filter(cn_celltypes == cn) %>%
    select(minor_cell_type, cn_n)
  
  # Get counts for the rest (combined data)
  rest_data <- total_cd4_counts %>%
    left_join(cn_data, by = "minor_cell_type") %>%
    mutate(
      cn_n = replace_na(cn_n, 0),
      rest_n = total_n - cn_n
    )
  ...
  contingency_table <- matrix(
    ...
    dimnames = list(
      Group = c(paste0("CN_", cn), "Combined"),
      Type = rest_data$minor_cell_type
    )
  )
  ...
  # Store the result
  cd4_fisher_results[[as.character(cn)]] <- test_result
...
# Collect p-values into a data frame
cd4_fisher_p_values <- data.frame(
  cn_celltypes = names(cd4_fisher_results),
  p_value = sapply(cd4_fisher_results, function(x) x$p.value)
)

# Convert 'cn_celltypes' to character for consistency
cd4_fisher_p_values$cn_celltypes <- as.character(cd4_fisher_p_values$cn_celltypes)

# Adjust p-values using FDR
cd4_fisher_p_values$adjusted_p_value <- p.adjust(cd4_fisher_p_values$p_value, method = "fdr")

cd4_fisher_p_values$labely <- 103

# Create the plot
p_cd4_fractions <- cd4_fractions |>
  filter(cn_celltypes %in% c("All", "0", "8", "9", "11")) |>
  mutate(cn_celltypes = factor(cn_celltypes, levels = c("All", "0", "8", "9", "11"))) |>
  ggplot(aes(x = cn_celltypes, y = perc * 100)) +
  geom_bar(aes(fill = minor_cell_type), stat = 'identity') +
  scale_fill_manual(values = color_palette_minor_2) +
  labs(x = "preCN", y = "CD4 T cell\nSubpopulation [%]", fill = "Cell Type") +
  guides(fill = guide_legend(title = NULL)) +
  geom_vline(xintercept = 1.5, linetype = "dashed") +
  geom_text(
    data = cd4_fisher_p_values,
    aes(x = cn_celltypes, y = labely, label = paste0("p=", signif(adjusted_p_value, 2))),
    size = 3
  ) +
  scale_y_continuous(limits = c(0, 105), breaks = c(0, 25, 50, 75, 100), expand = expansion(mult = c(0, 0))) +
...
p_blborder10cn12_cd4 <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("CD4 Tnaive","pSTAT1neg Non Th17 CD4 Tmem", "pSTAT1pos Non Th17 CD4 Tmem",
                                           "pSTAT1neg Th17 CD4 Tmem", "pSTAT1pos Th17 CD4 Tmem", "CD4 Tex", "Treg"))|>
  mutate(perc = n/sum(n)) |>
  ggplot(aes(x=as.factor(cn_celltypes)  , y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD4 T cell\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  theme_classic()+
  theme(
    legend.position = "top",
...
p_blborder10cn12_cd4_Tnaive <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd4_Tex <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd4_Treg <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd4_Th17Tmem <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("CD4 Tnaive","pSTAT1neg Non Th17 CD4 Tmem", "pSTAT1pos Non Th17 CD4 Tmem",
                                           "pSTAT1neg Th17 CD4 Tmem", "pSTAT1pos Th17 CD4 Tmem", "CD4 Tex", "Treg"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type %in% c("pSTAT1neg Th17 CD4 Tmem", "pSTAT1pos Th17 CD4 Tmem"))|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD4 T cell\nsubpopulation [%]", fill = "Functional cell type")+
...
p_blborder10cn12_cd4_NonTh17Tmem <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("CD4 Tnaive","pSTAT1neg Non Th17 CD4 Tmem", "pSTAT1pos Non Th17 CD4 Tmem",
                                           "pSTAT1neg Th17 CD4 Tmem", "pSTAT1pos Th17 CD4 Tmem", "CD4 Tex", "Treg"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type %in% c("pSTAT1neg Non Th17 CD4 Tmem", "pSTAT1pos Non Th17 CD4 Tmem"))|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD4 T cell\nsubpopulation [%]", fill = "Functional cell type")+
...
##
## NEW CD8 ANALYSIS INCLUDING Chi2 TEST
##

# Original data processing
cn_cd8_fractions <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  group_by(cn_celltypes) %>%
  count(minor_cell_type) %>%
  filter(minor_cell_type %in% c("CD8 Tnaive","CD8 Tmem", "CD8 GZMB+ Tmem", "CD8 Tex")) %>%
  mutate(perc = n / sum(n))

# Create combined data
all_cd8_fractions <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  count(minor_cell_type) %>%
  filter(minor_cell_type %in% c("CD8 Tnaive","CD8 Tmem", "CD8 GZMB+ Tmem", "CD8 Tex")) %>%
  mutate(
...
# Combine original and combined data
cd8_fractions <- bind_rows(cn_cd8_fractions, all_cd8_fractions)

# Get total counts across all cn_celltypes
total_cd8_counts <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  filter(minor_cell_type %in% c("CD8 Tnaive","CD8 Tmem", "CD8 GZMB+ Tmem", "CD8 Tex")) %>%
  count(minor_cell_type) %>%
  rename(total_n = n)
...
# Get counts per cn_celltypes
cn_cd8_counts <- bl_border10cn12_raw %>%
  filter(cn_celltypes != "NaN") %>%
  filter(minor_cell_type %in% c("CD8 Tnaive","CD8 Tmem", "CD8 GZMB+ Tmem", "CD8 Tex")) %>%
  group_by(cn_celltypes) %>%
  count(minor_cell_type) %>%
...
# Perform Fisher's exact test for CNS of interest
cn_list <- c("0","8","9","11")
cd8_fisher_results <- list()

for (cn in cn_list) {
  # Get counts for the current cn_celltypes
  cn_data <- cn_cd8_counts %>%
    filter(cn_celltypes == cn) %>%
    select(minor_cell_type, cn_n)
  
  # Get counts for the rest (combined data)
  rest_data <- total_cd8_counts %>%
    left_join(cn_data, by = "minor_cell_type") %>%
    mutate(
      cn_n = replace_na(cn_n, 0),
      rest_n = total_n - cn_n
  ...
  # Store the result
  cd8_fisher_results[[as.character(cn)]] <- test_result
...
# Collect p-values into a data frame
cd8_fisher_p_values <- data.frame(
  cn_celltypes = names(cd8_fisher_results),
  p_value = sapply(cd8_fisher_results, function(x) x$p.value)
)

# Convert 'cn_celltypes' to character for consistency
cd8_fisher_p_values$cn_celltypes <- as.character(cd8_fisher_p_values$cn_celltypes)

# Adjust p-values using FDR
cd8_fisher_p_values$adjusted_p_value <- p.adjust(cd8_fisher_p_values$p_value, method = "fdr")

cd8_fisher_p_values$labely <- 103

# Create the plot
p_cd8_fractions <- cd8_fractions |>
  filter(cn_celltypes %in% c("All", "0", "8", "9", "11")) |>
  mutate(cn_celltypes = factor(cn_celltypes, levels = c("All", "0", "8", "9", "11"))) |>
  ggplot(aes(x = cn_celltypes, y = perc * 100)) +
  geom_bar(aes(fill = minor_cell_type), stat = 'identity') +
  scale_fill_manual(values = color_palette_minor_2) +
  labs(x = "preCN", y = "CD8 T cell\nSubpopulation [%]", fill = "Cell Type") +
  guides(fill = guide_legend(title = NULL)) +
  geom_vline(xintercept = 1.5, linetype = "dashed") +
  geom_text(
    data = cd8_fisher_p_values,
    aes(x = cn_celltypes, y = labely, label = paste0("p=", signif(adjusted_p_value, 2))),
    size = 3
  ) +
  scale_y_continuous(limits = c(0, 105), breaks = c(0, 25, 50, 75, 100), expand = expansion(mult = c(0, 0))) +
...
p_blborder10cn12_cd8 <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD8 T cell\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme(
    legend.position = "top",
...
p_blborder10cn12_cd8_Tnaive <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd8_Tex <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd8_gzmbneg_Tmem <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("CD8 Tnaive","pSTAT1pos CD8 Tmem","pSTAT1neg CD8 Tmem", "pSTAT1neg CD8 GZMBpos Tmem","pSTAT1pos CD8 GZMBpos Tmem", "CD8 Tex"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type %in% c("pSTAT1pos CD8 Tmem", "pSTAT1neg CD8 Tmem"))|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD8 T cell\nsubpopulation [%]", fill = "Functional cell type")+
...
# p_blborder10cn12_cd8_neg_neg_Tmem <-bl_border10cn12_raw |>
#    filter(cn_celltypes != "NaN")|>
#   # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
#   group_by(cn_celltypes)|>
#   count(functional_minor_cell_type)|>
...
p_blborder10cn12_cd8_gzmbpos_Tmem <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("CD8 Tnaive","pSTAT1pos CD8 Tmem","pSTAT1neg CD8 Tmem", "pSTAT1neg CD8 GZMBpos Tmem","pSTAT1pos CD8 GZMBpos Tmem", "CD8 Tex"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type %in% c("pSTAT1pos CD8 GZMBpos Tmem", "pSTAT1neg CD8 GZMBpos Tmem"))|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="CD8 T cell\nsubpopulation [%]", fill = "Functional cell type")+
...
# p_blborder10cn12_cd8_pos_pos_Tmem <- bl_border10cn12_raw |>
#    filter(cn_celltypes != "NaN")|>
#   # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
#   group_by(cn_celltypes)|>
#   count(functional_minor_cell_type)|>
...
p_blborder10cn12_nks <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="NK cell\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  coord_flip()+
  theme_classic()+
...
p_blborder10cn12_neg_dim_nk <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_neg_bright_nk <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_pos_dim_nk <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_pos_bright_nk <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_macs <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="Macrophage\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  coord_flip()+
  theme_classic() 

p_blborder10cn12_M1mac <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_M2mac <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_monos <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="Monocyte\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  coord_flip()+
  theme_classic()

p_blborder10cn12_class_mono <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_itm_mono <-bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_nonclass_mono <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_adipo <-bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  ...
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="Other cell\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme_classic()+
  theme(
...
p_blborder10cn12_MKs <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_ECs <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_MDSCs <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_MoDC <- bl_border10cn12_raw |>
   filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
p_blborder10cn12_bcell <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("MBCs", "B cells"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type == "B cells")|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="B cell\nsubpopulation [%]", fill = "Functional cell type")+
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme_classic()+
  theme(
...
p_blborder10cn12_MBC <- bl_border10cn12_raw |>
  filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
  filter(functional_minor_cell_type %in% c("MBCs", "B cells"))|>
  mutate(perc = n/sum(n)) |>
  filter(functional_minor_cell_type == "MBCs")|>
  ggplot(aes(x=as.factor(reorder(cn_celltypes, -perc)), y = perc*100, fill = functional_minor_cell_type))+
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_palette_2)+
  labs(x="Cell neighborhood", y="B cell\nsubpopulation [%]", fill = "Functional cell type")+
...
# Retrieve the plots using the pattern
pattern_composition_blborder10cn12 <- "p_blborder10cn12_[a-zA-Z]"
plots_composition <- mget(ls(pattern = pattern_composition_blborder10cn12))

# Define the directory to save the plots
output_dir <- "blborder10cn12_composition"
dir.create(output_dir, showWarnings = FALSE)
...
save_plot(plots_composition, output_dir, width = 3.5, height = 3)

# Cell compositions per neighborhood saved as tables including percentages

bl_border10cn12_cell_composition_table <- bl_border10cn12_raw |>
  # filter(cn_celltypes != "NaN")|>
  # mutate(cn_celltypes = factor(cn_celltypes, levels = c(0, 11, 8, 9))) |>
  group_by(cn_celltypes)|>
  count(functional_minor_cell_type)|>
...
write.csv(bl_border10cn12_cell_composition_table, "bl_border10cn12_cell_composition_table.csv")
...
# fit <- crr(ftime = survival_bl_border10_cn12$CRS_onset_ci, fstatus = survival_bl_border10_cn12$CRS_1, cov1 = survival_bl_border10_cn12$X0)
# 
# # Print the summary
# summary(fit)
...
##
## Grading of cytopenias following CTCAE v5.0
##

# Function to grade anemia based on HgB values
grade_anemia <- function(hgb_values) {
  sapply(hgb_values, function(hgb) {
    if (is.na(hgb)) {
      return(NA)
    } else if (hgb >= 13.5) {
      return(0)  # No anemia
    } else if (hgb >= 10.0 & hgb < 13.5) {
      return(1)  # Grade 1
    } else if (hgb >= 8.0 & hgb < 10.0) {
      return(2)  # Grade 2
    } else if (hgb >= 6.5 & hgb < 8.0) {
      return(3)  # Grade 3
    } else if (hgb < 6.5) {
      return(4)  # Grade 4
    } else {
...
# Adding anemia grading to the data frame
survival_bl_border10_cn12$HgB_d0_grade <- grade_anemia(survival_bl_border10_cn12$HgB_d0)
survival_bl_border10_cn12$HgB_d7_grade <- grade_anemia(survival_bl_border10_cn12$HgB_d7)
survival_bl_border10_cn12$HgB_d30_grade <- grade_anemia(survival_bl_border10_cn12$HgB_d30)
survival_bl_border10_cn12$HgB_d60_grade <- grade_anemia(survival_bl_border10_cn12$HgB_d60)
survival_bl_border10_cn12$HgB_d90_grade <- grade_anemia(survival_bl_border10_cn12$HgB_d90)
survival_bl_border10_cn12$HgB_LD_grade <- grade_anemia(survival_bl_border10_cn12$HgB_LD)


p_hemato_border10cn12_HGBLD_1 <-survival_bl_border10_cn12|>
  mutate(HgB_LD_grade_grouped = case_when(
    HgB_LD_grade == 0 ~ "0",
    HgB_LD_grade %in% 1:2 ~ "1-2",
    HgB_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_LD_grade_grouped = factor(HgB_LD_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=HgB_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  ...
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           ...
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Anemia CTCAE", y = "Area [norm.]", title = "LD")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_HGBLD_2 <-survival_bl_border10_cn12|>
  mutate(HgB_LD_grade_grouped = case_when(
    HgB_LD_grade %in% 0:1 ~ "0-1",
    HgB_LD_grade %in% 2 ~ "2",
    HgB_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_HGBLD_3 <-survival_bl_border10_cn12|>
  mutate(HgB_LD_grade_grouped = case_when(
    HgB_LD_grade %in% 0:1 ~ "0-1",
    HgB_LD_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_LD_grade_grouped = factor(HgB_LD_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=HgB_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = '#DDA0DD')+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_HGBd0_1 <-survival_bl_border10_cn12|>
  mutate(HgB_d0_grade_grouped = case_when(
    HgB_d0_grade == 0 ~ "0",
    HgB_d0_grade %in% 1:2 ~ "1-2",
    HgB_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_d0_grade_grouped = factor(HgB_d0_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=HgB_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = '#DDA0DD')+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Anemia CTCAE", y = "Area [norm.]", title = "D0")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_HGBd0_2 <-survival_bl_border10_cn12|>
  mutate(HgB_d0_grade_grouped = case_when(
    HgB_d0_grade %in% 0:1 ~ "0-1",
    HgB_d0_grade %in% 2 ~ "2",
    HgB_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_HGBd0_3 <-survival_bl_border10_cn12|>
  mutate(HgB_d0_grade_grouped = case_when(
    HgB_d0_grade %in% 0:1 ~ "0-1",
    HgB_d0_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_d0_grade_grouped = factor(HgB_d0_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=HgB_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = '#DDA0DD')+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_HGBd7_1 <-survival_bl_border10_cn12|>
  mutate(HgB_d7_grade_grouped = case_when(
    HgB_d7_grade == 0 ~ "0",
    HgB_d7_grade %in% 1:2 ~ "1-2",
    HgB_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_d7_grade_grouped = factor(HgB_d7_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=HgB_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = '#DDA0DD')+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Anemia CTCAE", y = "Area [norm.]", title = "D7")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_HGBd7_2 <-survival_bl_border10_cn12|>
  mutate(HgB_d7_grade_grouped = case_when(
    HgB_d7_grade %in% 0:1 ~ "0-1",
    HgB_d7_grade %in% 2 ~ "2",
    HgB_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_HGBd7_3 <-survival_bl_border10_cn12|>
  mutate(HgB_d7_grade_grouped = case_when(
    HgB_d7_grade %in% 0:1 ~ "0-1",
    HgB_d7_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(HgB_d7_grade_grouped = factor(HgB_d7_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=HgB_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = '#DDA0DD')+
  geom_pwc(method = "wilcox_test", 
...
p_hemato_border10cn12_HGBd30_1 <-survival_bl_border10_cn12|>
...
p_hemato_border10cn12_HGBd30_2 <-survival_bl_border10_cn12|>
  mutate(HgB_d30_grade_grouped = case_when(
...
p_hemato_border10cn12_HGBd30_3 <-survival_bl_border10_cn12|>
  mutate(HgB_d30_grade_grouped = case_when(
    HgB_d30_grade %in% 0:1 ~ "0-1",
    HgB_d30_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_HGBd60_1 <-survival_bl_border10_cn12|>
...
p_hemato_border10cn12_HGBd60_2 <-survival_bl_border10_cn12|>
  mutate(HgB_d60_grade_grouped = case_when(
...
p_hemato_border10cn12_HGBd60_3 <-survival_bl_border10_cn12|>
  mutate(HgB_d60_grade_grouped = case_when(
    HgB_d60_grade %in% 0:1 ~ "0-1",
    HgB_d60_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_HGBd90_1 <-survival_bl_border10_cn12|>
...
p_hemato_border10cn12_HGBd90_2 <-survival_bl_border10_cn12|>
  mutate(HgB_d90_grade_grouped = case_when(
...
p_hemato_border10cn12_HGBd90_3 <-survival_bl_border10_cn12|>
  mutate(HgB_d90_grade_grouped = case_when(
    HgB_d90_grade %in% 0:1 ~ "0-1",
    HgB_d90_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
# Renaming Platelets into PLT consistently
colnames(survival_bl_border10_cn12)<- gsub("Platelets_d7", "PLT_d7", colnames(survival_bl_border10_cn12))
colnames(survival_bl_border10_cn12)<- gsub("Platelets_d30", "PLT_d30", colnames(survival_bl_border10_cn12))
colnames(survival_bl_border10_cn12)<- gsub("Platelets_d60", "PLT_d60", colnames(survival_bl_border10_cn12))
colnames(survival_bl_border10_cn12)<- gsub("Platelets_d90", "PLT_d90", colnames(survival_bl_border10_cn12))

# Function to grade thrombocytopenia based on PLT values
grade_thrombocytopenia <- function(plt_values) {
  sapply(plt_values, function(plt) {
    if (is.na(plt)) {
      return(NA)
    } else if (plt >= 150) {
      return(0)  # No thrombocytopenia
    } else if (plt >= 75 & plt < 150) {
      return(1)  # Grade 1
    } else if (plt >= 50 & plt < 75) {
      return(2)  # Grade 2
    } else if (plt >= 25 & plt < 50) {
      return(3)  # Grade 3
    } else if (plt < 25) {
      return(4)  # Grade 4
    } else {
...
# Apply grading function to each PLT column (you should replace `plt_column` with actual column names)
survival_bl_border10_cn12$PLT_d0_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_d0)
survival_bl_border10_cn12$PLT_d7_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_d7)
survival_bl_border10_cn12$PLT_d30_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_d30)
survival_bl_border10_cn12$PLT_d60_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_d60)
survival_bl_border10_cn12$PLT_d90_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_d90)
survival_bl_border10_cn12$PLT_LD_grade <- grade_thrombocytopenia(survival_bl_border10_cn12$PLT_LD)

p_hemato_border10cn12_PLTLD_1 <-survival_bl_border10_cn12|>
  mutate(PLT_LD_grade_grouped = case_when(
    PLT_LD_grade == 0 ~ "0",
    PLT_LD_grade %in% 1:2 ~ "1-2",
    PLT_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_LD_grade_grouped = factor(PLT_LD_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=PLT_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  ...
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           ...
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Thrombocytopenia CTCAE", y = "Area [norm.]", title = "LD")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_PLTLD_2 <-survival_bl_border10_cn12|>
  mutate(PLT_LD_grade_grouped = case_when(
    PLT_LD_grade %in% 0:1 ~ "0-1",
    PLT_LD_grade %in% 2 ~ "2",
    PLT_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_PLTLD_3 <-survival_bl_border10_cn12|>
  mutate(PLT_LD_grade_grouped = case_when(
    PLT_LD_grade %in% 0:1 ~ "0-1",
    PLT_LD_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_LD_grade_grouped = factor(PLT_LD_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=PLT_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = "#17becf")+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_PLTd0_1 <-survival_bl_border10_cn12|>
  mutate(PLT_d0_grade_grouped = case_when(
    PLT_d0_grade == 0 ~ "0",
    PLT_d0_grade %in% 1:2 ~ "1-2",
    PLT_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_d0_grade_grouped = factor(PLT_d0_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=PLT_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = "#17becf")+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Thrombocytopenia CTCAE", y = "Area [norm.]", title = "D0")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_PLTd0_2 <-survival_bl_border10_cn12|>
  mutate(PLT_d0_grade_grouped = case_when(
    PLT_d0_grade %in% 0:1 ~ "0-1",
    PLT_d0_grade %in% 2 ~ "2",
    PLT_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_PLTd0_3 <-survival_bl_border10_cn12|>
  mutate(PLT_d0_grade_grouped = case_when(
    PLT_d0_grade %in% 0:1 ~ "0-1",
    PLT_d0_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_d0_grade_grouped = factor(PLT_d0_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=PLT_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = "#17becf")+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_PLTd7_1 <-survival_bl_border10_cn12|>
  mutate(PLT_d7_grade_grouped = case_when(
    PLT_d7_grade == 0 ~ "0",
    PLT_d7_grade %in% 1:2 ~ "1-2",
    PLT_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_d7_grade_grouped = factor(PLT_d7_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=PLT_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = "#17becf")+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Thrombocytopenia CTCAE", y = "Area [norm.]", title = "D7")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_PLTd7_2 <-survival_bl_border10_cn12|>
  mutate(PLT_d7_grade_grouped = case_when(
    PLT_d7_grade %in% 0:1 ~ "0-1",
    PLT_d7_grade %in% 2 ~ "2",
    PLT_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_PLTd7_3 <-survival_bl_border10_cn12|>
  mutate(PLT_d7_grade_grouped = case_when(
    PLT_d7_grade %in% 0:1 ~ "0-1",
    PLT_d7_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(PLT_d7_grade_grouped = factor(PLT_d7_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=PLT_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = "#17becf")+
  geom_pwc(method = "wilcox_test", 
...
p_hemato_border10cn12_PLTd30_1 <-survival_bl_border10_cn12|>
...
p_hemato_border10cn12_PLTd30_2 <-survival_bl_border10_cn12|>
  mutate(PLT_d30_grade_grouped = case_when(
...
p_hemato_border10cn12_PLTd30_3 <- survival_bl_border10_cn12|>
  mutate(PLT_d30_grade_grouped = case_when(
    PLT_d30_grade %in% 0:1 ~ "0-1",
    PLT_d30_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_PLTd60_1 <- survival_bl_border10_cn12|>
...
p_hemato_border10cn12_PLTd60_2 <- survival_bl_border10_cn12|>
  mutate(PLT_d60_grade_grouped = case_when(
...
p_hemato_border10cn12_PLTd60_3 <- survival_bl_border10_cn12|>
  mutate(PLT_d60_grade_grouped = case_when(
    PLT_d60_grade %in% 0:1 ~ "0-1",
    PLT_d60_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_PLTd90_1 <-survival_bl_border10_cn12|>
...
p_hemato_border10cn12_PLTd90_2 <-survival_bl_border10_cn12|>
  mutate(PLT_d90_grade_grouped = case_when(
...
p_hemato_border10cn12_PLTd90_3 <- survival_bl_border10_cn12|>
  mutate(PLT_d90_grade_grouped = case_when(
    PLT_d90_grade %in% 0:1 ~ "0-1",
    PLT_d90_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
# Function to grade neutropenia based on ANC values
grade_neutropenia <- function(anc_values) {
  sapply(anc_values, function(anc) {
    if (is.na(anc)) {
      return(NA)
    } else if (anc >= 1.92) {
      return(0)  # No neutropenia
    } else if (anc >= 1.5 & anc < 1.92) {
      return(1)  # Grade 1
    } else if (anc >= 1.0 & anc < 1.5) {
      return(2)  # Grade 2
    } else if (anc >= 0.5 & anc < 1.0) {
      return(3)  # Grade 3
    } else if (anc < 0.5) {
      return(4)  # Grade 4
    } else {
...
# Apply grading function to each ANC column (you should replace `anc_column` with actual column names)
survival_bl_border10_cn12$ANC_d0_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_d0)
survival_bl_border10_cn12$ANC_d7_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_d7)
survival_bl_border10_cn12$ANC_d30_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_d30)
survival_bl_border10_cn12$ANC_d60_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_d60)
survival_bl_border10_cn12$ANC_d90_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_d90)
survival_bl_border10_cn12$ANC_LD_grade <- grade_neutropenia(survival_bl_border10_cn12$ANC_LD)


p_hemato_border10cn12_ANCLD_1 <- survival_bl_border10_cn12|>
  mutate(ANC_LD_grade_grouped = case_when(
    ANC_LD_grade == 0 ~ "0",
    ANC_LD_grade %in% 1:2 ~ "1-2",
    ANC_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_LD_grade_grouped = factor(ANC_LD_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=ANC_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  ...
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           ...
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Neutropenia CTCAE", y = "Area [norm.]", title = "LD")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_ANCLD_2 <- survival_bl_border10_cn12|>
  mutate(ANC_LD_grade_grouped = case_when(
    ANC_LD_grade %in% 0:1 ~ "0-1",
    ANC_LD_grade %in% 2 ~ "2",
    ANC_LD_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_ANCLD_3 <- survival_bl_border10_cn12|>
  mutate(ANC_LD_grade_grouped = case_when(
    ANC_LD_grade %in% 0:1 ~ "0-1",
    ANC_LD_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_LD_grade_grouped = factor(ANC_LD_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=ANC_LD_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = 'darkblue')+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_ANCd0_1 <- survival_bl_border10_cn12|>
  mutate(ANC_d0_grade_grouped = case_when(
    ANC_d0_grade == 0 ~ "0",
    ANC_d0_grade %in% 1:2 ~ "1-2",
    ANC_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_d0_grade_grouped = factor(ANC_d0_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=ANC_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = 'darkblue')+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Neutropenia CTCAE", y = "Area [norm.]", title = "D0")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_ANCd0_2 <- survival_bl_border10_cn12|>
  mutate(ANC_d0_grade_grouped = case_when(
    ANC_d0_grade %in% 0:1 ~ "0-1",
    ANC_d0_grade %in% 2 ~ "2",
    ANC_d0_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_ANCd0_3 <- survival_bl_border10_cn12|>
  mutate(ANC_d0_grade_grouped = case_when(
    ANC_d0_grade %in% 0:1 ~ "0-1",
    ANC_d0_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_d0_grade_grouped = factor(ANC_d0_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=ANC_d0_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = 'darkblue')+
  geom_pwc(method = "wilcox_test",
...
p_hemato_border10cn12_ANCd7_1 <- survival_bl_border10_cn12|>
  mutate(ANC_d7_grade_grouped = case_when(
    ANC_d7_grade == 0 ~ "0",
    ANC_d7_grade %in% 1:2 ~ "1-2",
    ANC_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_d7_grade_grouped = factor(ANC_d7_grade_grouped, levels = c("0", "1-2", ">3"))) |>
  ggplot(aes(x=ANC_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = 'darkblue')+
  geom_pwc(method = "wilcox_test", ref.group = "0", 
           bracket.nudge.y = 0.05, 
           step.increase = 0.15, 
           tip.length = 0.01, label.size = 3)+
  labs(x = "Neutropenia CTCAE", y = "Area [norm.]", title = "D7")+
  facet_wrap(vars(area), scales = "free")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  theme_classic() +
  theme(
...
p_hemato_border10cn12_ANCd7_2 <- survival_bl_border10_cn12|>
  mutate(ANC_d7_grade_grouped = case_when(
    ANC_d7_grade %in% 0:1 ~ "0-1",
    ANC_d7_grade %in% 2 ~ "2",
    ANC_d7_grade > 2 ~ ">3",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
...
p_hemato_border10cn12_ANCd7_3 <- survival_bl_border10_cn12|>
  mutate(ANC_d7_grade_grouped = case_when(
    ANC_d7_grade %in% 0:1 ~ "0-1",
    ANC_d7_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
  pivot_longer(cols = all_of(bl_border10_cn12_variables), names_to = "area", values_to = "area_perc")|>
  filter(area != "nan")|>
  mutate(ANC_d7_grade_grouped = factor(ANC_d7_grade_grouped, levels = c("0-1", ">=2"))) |>
  ggplot(aes(x=ANC_d7_grade_grouped, y=area_perc))+
  geom_boxplot()+
  geom_jitter(size = 2, alpha = 0.3, width = 0.2, color = 'darkblue')+
  geom_pwc(method = "wilcox_test", 
...
p_hemato_border10cn12_ANCd30_1 <- survival_bl_border10_cn12|>
...
p_hemato_border10cn12_ANCd30_2 <- survival_bl_border10_cn12|>
  mutate(ANC_d30_grade_grouped = case_when(
...
p_hemato_border10cn12_ANCd30_3 <- survival_bl_border10_cn12|>
  mutate(ANC_d30_grade_grouped = case_when(
    ANC_d30_grade %in% 0:1 ~ "0-1",
    ANC_d30_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_ANCd60_1 <- survival_bl_border10_cn12|>
...
p_hemato_border10cn12_ANCd60_2 <- survival_bl_border10_cn12|>
  mutate(ANC_d60_grade_grouped = case_when(
...
p_hemato_border10cn12_ANCd60_3 <- survival_bl_border10_cn12|>
  mutate(ANC_d60_grade_grouped = case_when(
    ANC_d60_grade %in% 0:1 ~ "0-1",
    ANC_d60_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
p_hemato_border10cn12_ANCd90_1 <- survival_bl_border10_cn12|>
...
p_hemato_border10cn12_ANCd90_2 <- survival_bl_border10_cn12|>
  mutate(ANC_d90_grade_grouped = case_when(
...
p_hemato_border10cn12_ANCd90_3 <- survival_bl_border10_cn12|>
  mutate(ANC_d90_grade_grouped = case_when(
    ANC_d90_grade %in% 0:1 ~ "0-1",
    ANC_d90_grade > 1 ~ ">=2",
    TRUE ~ NA_character_  )) |>
...
# Example usage
pattern_hemato_border10cn12 <- "p_hemato_border10cn12_[a-zA-Z0-9]+_[0-9]"
plots_hemato_border10cn12 <- mget(ls(pattern = pattern_hemato_border10cn12))

# Define the directory to save the CRS plots
output_dir <- "border10cn12_hemato"
dir.create(output_dir, showWarnings = FALSE)

# Save the plots to the specified directory
save_plot(plots_hemato_border10cn12, output_dir, width =6, height =8)
...
for (variable in bl_border10_cn12_variables_wonan) {
  
  # Generate the survival plot
  p <- survival_bl_border10_cn12|>
    mutate(PLT_d90_grade_grouped = case_when(
      PLT_d90_grade %in% 0:1 ~ "0-1",
      PLT_d90_grade > 1 ~ ">=2",
      TRUE ~ NA_character_  ),
    ...
    stat_compare_means(aes(group = PLT_time_value), label = "p.format") + # Add statistical significance between groups
    ...
    guides(color = "none", fill = "none")+
    labs(x = "Thrombocytopenia [CTCAE]", y = "CN0 area [norm.]")+
    theme_classic()+
    theme(
      strip.background = element_blank(),
      strip.text = element_text(size = 10),    plot.title = element_text(size = 12, hjust = 0.5)  # Adjust the font size of facet labels if needed
  ...
  # Add the plot to the list
  p_plt_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_PLT", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_plt_blborder10cn12[[variable]], width = 4, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_anc_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_ANC", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_anc_blborder10cn12[[variable]], width = 4, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_plttx_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_PLTTx", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_plttx_blborder10cn12[[variable]], width = 2, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_sevplt_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_SevPLT", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_sevplt_blborder10cn12[[variable]], width = 2, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_gcsf_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_GCSF", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_gcsf_blborder10cn12[[variable]], width = 2, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_hgb_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_HgB", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_hgb_blborder10cn12[[variable]], width = 4, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_sevhgb_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_SevHgB", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_sevhgb_blborder10cn12[[variable]], width = 2, height = 4)
}
...
for (variable in bl_border10_cn12_variables_wonan) {
  ...
  # Add the plot to the list
  p_hgbtx_blborder10cn12[[variable]] <- p
  
  # Save the plot as a SVG file
  subset_file_name <- paste0("border10cn12_HgBTx", variable, ".svg")
  subset_file_path <- file.path(output_dir, subset_file_name)
  print(paste("Saving plot to:", subset_file_path))  # Debugging line
  
  # Save the plot as a file
  ggsave(filename = subset_file_path, plot = p_hgbtx_blborder10cn12[[variable]], width = 2, height = 4)
}


survival_bl_border10_cn12$PLT_d0_grade
